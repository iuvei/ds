引用 [Airbnb JavaScript Style Guide](https://github.com/yuche/javascript/blob/master/README.md)
...
## 项目特例汇总
  ...
  - [箭头函数 {} 和 return 省略](#8.2)

  - [Class](#constructors) 项目暂不使用class，推荐模块组合方式

  - [模块export](#10.3) `export { zip, flatten, camelCase, sumBy } from 'lodash'` 生产后的lodash会打包多余模块

  - 代码块，空白，逗号 使用vscode格式化工具prettier自动换行处理

  - [不使用分号，遇到两个 IIFE 合并时手动加](#semicolons)

  - [String到Number类型转换](#type-casting--coercion) '3'直接使用+'3'转换

  - [文件命名规则](#naming-conventions) nuxt项目中pages使用下划线_,components组件使用驼峰camelCase,其他文件使用-连接符

  - [jQuery](#25.1) 项目尽可能不使用jQuery，使用 `$` 作为存储 vue $refs DOM 节点对象的变量名前缀
...
## 箭头函数
  ...
  ```
  > alpha注：可尽量省略return和花括号，使用vscode格式化工具prettier自动换行处理
...
## 模块
  ...
  > alpha注： 测试 `export { zip, flatten, camelCase, sumBy } from 'lodash'` 生产后的lodash会打包多余模块
...
<a name="blocks"></a>
## 代码块
  > alpha注：可不用大括号，使用vscode格式化工具prettier自动换行处理
...
## 空白
  ...
  > alpha注：使用vscode格式化工具prettier自动换行处理
...
## 分号
  ...
  > alpha注：目前项目中不使用分号，遇到两个 IIFE 合并时手动加`；`
...
## 类型转换
  ...
  - [21.3](#21.3) <a name='21.3'></a> 对数字使用 `parseInt` 转换，并带上类型转换的基数。
    ...
    > alpha注：目前项目中为了代码简洁直接使用+inputValue，具体情况再讨论
...
## 命名规则
  ...
  > alpha注：nuxt项目中pages使用下划线_,components组件使用驼峰camelCase,其他文件使用-连接符

  > [vue组件 camelCase vs. kebab-case](https://cn.vuejs.org/v2/guide/components.html#camelCase-vs-kebab-case)
